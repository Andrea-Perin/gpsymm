...
results = np.empty( (len(ANGLES), N_TESTS, CLASSES_PER_TEST, 2) )
for angle_idx, nangles in enumerate(ANGLES):
    ...
    for iteration, key in tqdm(zip(range(N_TESTS), test_keys)):
        ...
        preds = peelmap(circulant_predict, 4)(ckernels[..., 0])
        # remove the second diagonal
        preds, ps = ein.pack( [jnp.delete(p, i, axis=0) for i, p in enumerate(preds)], '* cb sa sb' )
        #
        avg_pred_sa = ein.reduce(preds, 'ca cb sa sb -> ca cb sa', 'mean')
        bin_pred = (avg_pred_sa > 0.)
        avg_bin_pred = ein.reduce(bin_pred, 'ca cb sa -> ca sa', jnp.all)
        avg_sp_err = 1 - ein.reduce(avg_bin_pred.astype(float), 'ca sa -> ca', 'mean')

        # WE ARE NOT DOING THIS ANYMORE BECAUSE IT DOES NOT WORK
        # solve regression
        regression_preds = np.empty( (CLASSES_PER_TEST, NUM_SEEDS) )
        for cls in range(CLASSES_PER_TEST):
            ...
            preds = ein.einsum(sol, ys, 'train test, train d -> test d')
            regression_preds[cls] = preds.flatten()

        bin_pred = (regression_preds > 0.)
        avg_reg_err = 1 - ein.reduce(bin_pred.astype(float), 'ca sa -> ca', 'mean')

        # log results
        results[angle_idx, iteration, :, 0] = avg_sp_err
        results[angle_idx, iteration, :, 1] = avg_reg_err

# %%
res_to_plot = ein.reduce(results, 'angle test class type -> angle type', 'mean')
fig, ax = plt.subplots()
ax.plot(ANGLES, res_to_plot[:, 0], label='spectral')
ax.plot(ANGLES, res_to_plot[:, 1], label='emp NTK')
plt.show()
...
for idx, ax in enumerate(axs.flatten()):
    ax.scatter(results[:, idx, 0], results[:, idx, 1], alpha=.1)
...
# errs = peelmap(circulant_error, 2)(avg_kc)
# sp_err = (errs[0, 1], errs[1, 0])
